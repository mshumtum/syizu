...
function* getOneToOneChatDetail(action) {
  getToken();
  var act = action;

  const {payload, messageCallback} = action.payload;
  ...
  setTimeout(async () => {
    try {
      ...
      if (token != '') {
        var url = `${baseUrl}getOneToOneChatDetail?_id=${payload.roomID}`;
...
export default function* getOneToOneChatDetailSaga() {
  yield takeEvery(GET_ONE_TO_ONE_CHAT_DETAIL, getOneToOneChatDetail);
}
