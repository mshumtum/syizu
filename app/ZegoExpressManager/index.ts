...
export class ZegoExpressManager {
  ...
  enableSpeaker(enable: boolean): Promise<void> {
    ...
    return ZegoExpressEngine.instance()
      ...
      .then(() => {
        console.warn(
          '[ZEGOCLOUD LOG][Manager][enableSpeaker] - Enable success',
          enable,
        );
      });
  }
  ...
  setLocalVideoView(renderView: number) {
    ...
    if (renderView === null) {
      console.error(
        'ZEGO RN LOG - setLocalVideoView: You need to pass in the correct element',
      );
      return;
    }
  ...
  setRemoteVideoView(userID: string, renderView: number) {
    if (renderView === null) {
      console.error(
        'ZEGO RN LOG - setRemoteVideoView: You need to pass in the correct element',
      );
      return;
    }
    if (!userID) {
